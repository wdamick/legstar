...
public abstract class CobolElementVisitor {
	...
	public final int getCurrentOccurs(
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Getting current size for array " + ce.getJavaName());
		}
		...
		if (ce.getDependingOn() != null && ce.getDependingOn().length() > 0) {
			...
			if (value == null) {
				throw new HostException("Cannot find ODO object "
						+ ce.getDependingOn()
						+ " for variable size array " + ce.getJavaName());
			}
			if (LOG.isDebugEnabled()) {
				LOG.debug("Variable size array dimension is "
						+ value.intValue());
			}
			return value.intValue();
		}
		if (LOG.isDebugEnabled()) {
			LOG.debug("Fixed size array dimension is "
					+ ce.getMaxOccurs());
		}
	...
	public final void storeCustomVariable(
		...
		getVariablesMap().put(
				binding.getJavaName(),
				binding.getObjectValue(binding.getJavaType()));
		
	}

	/**
	 * Store the value of a binding in the Object Depending On map. This
	 * will allow us to process the eaxct number of items for variable
	 * size arrays.
	 * @param binding the current numeric binding
	 * @throws HostException if value cannot be stored
	 */
	public final void storeODOValue(
			final ICobolNumericBinding binding) throws HostException {
		getODOMap().put(binding.getCobolName(), binding.getBigDecimalValue());
