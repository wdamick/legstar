...
public class CobolOctetStreamSimpleConverter extends CobolSimpleConverter
  implements ICobolOctetStreamConverter {
	...
	public final int toHost(
		...
		try {
			for (byte[] javaSource : ce.getValue()) {
	...
	public static final int toHostSingle(
		...
		// If input byte array is null, then stay on the same offset
		if (javaBytes == null) {
			return offset;
		}
		 ...
		 */
		for (int i = 0; i < javaBytes.length && i < cobolByteLength; i++) {
			hostTarget[offset + i] = javaBytes[i];
		}
		return lastOffset;
	}
