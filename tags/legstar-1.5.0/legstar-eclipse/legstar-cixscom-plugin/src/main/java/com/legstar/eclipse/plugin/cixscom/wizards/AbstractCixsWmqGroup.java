...
public abstract class AbstractCixsWmqGroup extends AbstractCixsControlsGroup {
    ...
    public boolean validateControls() {
        ...
        if (getWmqJndiContextFactory() == null
                || getWmqJndiContextFactory().length() == 0) {
            getWizardPage().updateStatus(
                    Messages.invalid_wmq_jndi_context_factory_msg);
            return false;
        }
        if (getWmqConnectionFactory() == null
                || getWmqConnectionFactory().length() == 0) {
            getWizardPage().updateStatus(
                    Messages.invalid_wmq_connection_factory_msg);
            return false;
        }
        if (getWmqZosQueueManager() == null
                || getWmqZosQueueManager().length() == 0) {
            getWizardPage().updateStatus(
                    Messages.invalid_wmq_zos_queue_manager_msg);
            return false;
        }
        ...
        if (getWmqReplyQueue() == null || getWmqReplyQueue().length() == 0) {
            getWizardPage().updateStatus(
                    Messages.invalid_wmq_target_reply_queue_msg);
            return false;
        }
        return true;
    }
    ...
    /**
     * @param wmqJndiContextFactory context factory class JBossESB uses to do
     *            naming lookups for WMQ resources
     */
    ...
    /**
     * @return connection-factory used to lookup queues/topics in a naming
     *         directory (JNDI)
     */
    ...
    /**
     * @param wmqConnectionFactory connection-factory used to lookup
