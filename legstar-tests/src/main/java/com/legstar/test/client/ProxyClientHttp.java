...
/**
 * Simulates the behavior of a Mainframe program consuming cultureinfo.
 */
public class ProxyClientHttp {
     ...
     * @throws ProxyClientException if invoke fails
     */
    public String invoke(
            final String mainframeRequestData,
            final int expectedResponseSize) throws ProxyClientException {
        try {
            if (mHttpClient == null) {
            ...
            if (expectedResponseSize != postMethod.getResponseContentLength()) {
                throw new ProxyClientException("Content length returned does not match");
            }
