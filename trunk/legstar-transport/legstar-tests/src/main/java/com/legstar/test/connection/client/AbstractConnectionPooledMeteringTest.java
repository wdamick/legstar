 ...
 * <ul>
 * <li>Normal single pass JUnit test in which case setUp/tearDown are executed
 * and oneTimeSetUp/OneTimeTearDown are ignored</li>
...
public abstract class AbstractConnectionPooledMeteringTest extends TestCase {
    ...
    /**
     * {@inheritDoc} Make sure JMeter does not run this. This is only useful
     * when run locally.
     */
    ...
    public void testLsfileae() throws Exception {
        LegStarRequest request = AbstractConnectionTester
                .getLsfileaeRequest100(
    ...
    /**
     * Submit a request for T1VOLUMEB execution, wait for the reply and check
     * it.
     * 
     * @throws Exception if test fails
     */
    public void testT1Volumeb() throws Exception {
        LegStarRequest request = AbstractConnectionTester.createLargeRequestB(
                new LegStarAddress(getEndpointName()));
        EngineHolder.getEngine().addRequest(request);
        request.await(DEFAULT_INVOKE_TIMEOUT_MSEC, TimeUnit.MILLISECONDS);
        if (request.getException() != null) {
