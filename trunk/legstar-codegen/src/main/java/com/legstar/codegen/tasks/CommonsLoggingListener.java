...
/**
 * This is a clone of Apache CommonsLoggingListener.
 * We could have embarked ant-commons-logging.jar but it is part of ant 1.7.0 and above.
 * The issue is that the ant libraries are being loaded from a different classloader
 * than our ant task and since we provide commons-logging (which is not part
 * of ant) then there is a NoClassDefFoundError: org/apache/commons/logging/LogConfigurationException.
 * */
    ...
    public void targetFinished(BuildEvent event) {
        if (initialized) {
            ...
            Log log = getLog("org.apache.tools.ant.Target",
                    event.getTarget().getName());
            if (event.getException() == null) {
                realLog(log, "Target end: " + targetName, Project.MSG_DEBUG,
                        null);
            } else {
