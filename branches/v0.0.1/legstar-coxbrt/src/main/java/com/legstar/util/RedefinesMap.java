...
import com.legstar.coxb.reflect.CChoiceBinding;
...
public class RedefinesMap {
	...
	private static
	  ThreadLocal < Hashtable < String, CChoiceBinding > > mapHolder
	  = new ThreadLocal < Hashtable < String, CChoiceBinding > >();
	 ...
	 */
	public final synchronized CChoiceBinding getChoiceElement(
			final String redefines)	{
		
		CChoiceBinding value = getRedefinesMap().get(redefines);
		return value;
	}
	 ...
	 */
	public final Hashtable < String, CChoiceBinding >
		getRedefinesMap()	{

		Hashtable < String, CChoiceBinding > result = mapHolder.get();
		if (result == null) {
			mapHolder.set(new Hashtable < String, CChoiceBinding >());
		}
